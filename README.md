...
> 版本號的單一來源是 [`src/__init__.py`](src/__init__.py) 的 `__version__`。
> GUI 右下角從那裡讀，本頁標題與下方 changelog 要一起更新。
> **`v3.x` 與 `1.0.x` 是兩套不連續的編號**，後者是 micro:bit 原型時代的，別對齊。
---
## 簡介
- 火箭系統的地面端程式
- 使用 PyQt 製作 GUI
...
### v3.5
```
v3.5.0  (2026-07-30 ~ 2026-08-01)   ← 含發射日全部修復，共 27 個 commit

【2026-08-01 發射日當天】
單一 KH 突波會永久灌水回報的頂點高度（峰值閂鎖沒有排離群值）
任何在發射台上的震動都可能把 IGNITION 卡住
飛行時間軸改用火箭的時鐘，不是地面站的接收時間
...
圖表事件標籤全部塌成「emoji + ch」
自動釋放的 hook 只看得到焦點通道
畫面上的 "TextLabel"，以及背後沒有資料卻亮綠燈
隱藏 port 重試應該是收合，不是消音
上行指令 token 沒有跨 repo 測試，而且其中一個根本送不出去
三條在別的東西已經出錯之後會卡住的 teardown 路徑
姿態掛載軸依實測釘死 -x，拿掉自動偵測

【2026-07-31】
偵測 IMU 掛載軸，不再假設板子是平放的
log widget 沒有上限，補上
地圖需要網路，而旭海可能沒有
氣囊移除 —— abg 按鈕、指令、確認一併拿掉
raw log 不再卡住 reader；/dpl 只在上升段問兩次

【2026-07-30】
不再追蹤 settings.json；architecture.md 對齊實際程式
啟動時重建佇列，避免殘留 sentinel 毒死下一個 parser
階段列會在傘出來的那一刻顯示 IGNITION

★ 版本號改為單一來源 src/__init__.py（GUI 原本寫死 v1.0.5，
  README 寫 v3.4，GitHub Release 停在 v3.0 —— 三個都不對）
```
